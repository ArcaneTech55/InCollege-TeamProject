...
6110-GET-OPTIONAL-EXP.
    ...
    PERFORM VARYING J FROM 1 BY 1 UNTIL J > 3 OR WS-USER-WANT-TO-EXIT
        ...
        *> --- Title ---
        PERFORM UNTIL FUNCTION LENGTH(FUNCTION TRIM(WS-GENERIC-INPUT)) > 0 OR WS-USER-WANT-TO-EXIT
            MOVE WS-BLANK-INPUT-MSG TO DISPLAY-MSG
            PERFORM 8000-DISPLAY-ROUTINE
            READ INPUT-FILE INTO WS-GENERIC-INPUT
                AT END SET WS-USER-WANT-TO-EXIT TO TRUE
        ...
        *> --- Company ---
        STRING "Experience #" J " - Company/Organization:" DELIMITED BY SIZE INTO DISPLAY-MSG
        PERFORM 8000-DISPLAY-ROUTINE
        PERFORM UNTIL FUNCTION LENGTH(FUNCTION TRIM(WS-GENERIC-INPUT)) > 0 OR WS-USER-WANT-TO-EXIT
            READ INPUT-FILE INTO WS-GENERIC-INPUT
                AT END SET WS-USER-WANT-TO-EXIT TO TRUE
        ...
        *> --- Dates ---
        STRING "Experience #" J " - Dates (e.g., Summer 2024):" DELIMITED BY SIZE INTO DISPLAY-MSG
        PERFORM 8000-DISPLAY-ROUTINE
        PERFORM UNTIL FUNCTION LENGTH(FUNCTION TRIM(WS-GENERIC-INPUT)) > 0 OR WS-USER-WANT-TO-EXIT
            READ INPUT-FILE INTO WS-GENERIC-INPUT
                AT END SET WS-USER-WANT-TO-EXIT TO TRUE
        ...
        *> --- Description ---
        STRING "Experience #" J " - Description (optional):" DELIMITED BY SIZE INTO DISPLAY-MSG
        PERFORM 8000-DISPLAY-ROUTINE
        READ INPUT-FILE INTO WS-GENERIC-INPUT
            AT END SET WS-USER-WANT-TO-EXIT TO TRUE
        END-READ
