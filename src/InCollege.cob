...
01 WS-CONNECTION-FLAGS.
       ...
       05 WS-REQ-SENT-BY-ME        PIC X VALUE 'N'.
           88 REQUEST-SENT-BY-ME       VALUE 'Y'.
       05 WS-REQ-SENT-BY-THEM      PIC X VALUE 'N'.
           88 REQUEST-SENT-BY-THEM     VALUE 'Y'.
       05 WS-USERNAME-EXISTS       PIC X VALUE 'N'.
           88 USER-FOUND               VALUE 'Y'.
           88 USER-NOT-FOUND           VALUE 'N'.

01 WS-PENDING-VIEW-FLAGS.
       05 WS-HAS-PENDING          PIC X VALUE 'N'.
...
5000-POST-LOGIN-MENU.
       PERFORM UNTIL WS-USER-WANT-TO-EXIT
           ...
           PERFORM 8000-DISPLAY-ROUTINE
           MOVE "7. View My Pending Connection Requests" TO DISPLAY-MSG
           PERFORM 8000-DISPLAY-ROUTINE
           ...
           EVALUATE WS-INPUT-CHOICE
               ...
               WHEN "7"
                   PERFORM 6500-VIEW-PENDING-REQUESTS
               WHEN OTHER
                   MOVE WS-INVALID-CHOICE TO DISPLAY-MSG
                   PERFORM 8000-DISPLAY-ROUTINE
           END-EVALUATE
...
6400-SEND-CONNECTION-REQUEST.
       ...
       *> Prevent sending request to self
       IF FUNCTION TRIM(WS-INPUT-USERNAME) = FUNCTION TRIM(WS-TO-CONNECTION-USERNAME)
           MOVE "You cannot send a connection request to yourself." TO DISPLAY-MSG
           PERFORM 8000-DISPLAY-ROUTINE
           GO TO SEND-CONNECTION-REQUEST-END
       END-IF
       ...
       IF REQUEST-SENT-BY-ME
           MOVE "You already sent a request to this user." TO DISPLAY-MSG
           PERFORM 8000-DISPLAY-ROUTINE
           GO TO SEND-CONNECTION-REQUEST-END
       END-IF

       IF REQUEST-SENT-BY-THEM
           MOVE "This user has already sent you a connection request." TO DISPLAY-MSG
           PERFORM 8000-DISPLAY-ROUTINE
           GO TO SEND-CONNECTION-REQUEST-END
       END-IF
...
6415-CHECK-PENDING-REQUEST.
          ...
          MOVE 'N' TO WS-REQ-SENT-BY-ME WS-REQ-SENT-BY-THEM
          ...
          PERFORM UNTIL WS-EOF-FLAG OR REQUEST-FOUND
               READ PENDING-REQUEST-FILE
                   AT END
                       SET WS-EOF-FLAG TO TRUE
                   NOT AT END
                      IF FUNCTION TRIM(PEND-SENDING-USER) = FUNCTION TRIM(WS-INPUT-USERNAME)
                         AND FUNCTION TRIM(PEND-RECEIVING-USER) = FUNCTION TRIM(WS-TO-CONNECTION-USERNAME)
                          SET REQUEST-FOUND TO TRUE
                          SET REQUEST-SENT-BY-ME TO TRUE
                      END-IF

                      IF FUNCTION TRIM(PEND-SENDING-USER) = FUNCTION TRIM(WS-TO-CONNECTION-USERNAME)
                         AND FUNCTION TRIM(PEND-RECEIVING-USER) = FUNCTION TRIM(WS-INPUT-USERNAME)
                          SET REQUEST-FOUND TO TRUE
                          SET REQUEST-SENT-BY-THEM TO TRUE
                      END-IF
               END-READ
           END-PERFORM
...
6500-VIEW-PENDING-REQUESTS.
       MOVE "--- Pending Connection Requests ---" TO DISPLAY-MSG
       PERFORM 8000-DISPLAY-ROUTINE
       ...
       PERFORM UNTIL WS-EOF-FLAG
           READ PENDING-REQUEST-FILE
               AT END
                   SET WS-EOF-FLAG TO TRUE
               NOT AT END
                   IF FUNCTION TRIM(PEND-RECEIVING-USER) = FUNCTION TRIM(WS-CURRENT-USER)
                       SET PENDING-FOUND TO TRUE
                       STRING "From: " FUNCTION TRIM(PEND-SENDING-USER)
                           DELIMITED BY SIZE INTO DISPLAY-MSG
                       PERFORM 8000-DISPLAY-ROUTINE
                   END-IF
           END-READ
       ...
       IF NO-PENDING
           MOVE "You have no pending connection requests at this time." TO DISPLAY-MSG
           PERFORM 8000-DISPLAY-ROUTINE
       END-IF
