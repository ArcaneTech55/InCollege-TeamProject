...
01 UP-GRAD-YEAR-INVALID     PIC X(60) VALUE 'Graduation year must be 1900-2100.'.
...
*> For experience prompt numbering
01 WS-J-TEXT                PIC ZZ9.
...
6105-GET-REQUIRED-FIELDS.
    ...
    MOVE WS-TEMP-FIELD TO UP-FIRST-NAME

    MOVE WS-ENTER-LAST TO DISPLAY-MSG
    PERFORM 6205-GET-VALID-REQUIRED-FIELDS
    IF WS-USER-WANT-TO-EXIT EXIT PARAGRAPH END-IF
    MOVE WS-TEMP-FIELD TO UP-LAST-NAME

    MOVE WS-ENTER-UNIV TO DISPLAY-MSG
    PERFORM 6205-GET-VALID-REQUIRED-FIELDS
    IF WS-USER-WANT-TO-EXIT EXIT PARAGRAPH END-IF
    MOVE WS-TEMP-FIELD TO UP-UNIVERSITY

    MOVE WS-ENTER-MAJOR TO DISPLAY-MSG
    PERFORM 6205-GET-VALID-REQUIRED-FIELDS
    IF WS-USER-WANT-TO-EXIT EXIT PARAGRAPH END-IF
    MOVE WS-TEMP-FIELD TO UP-MAJOR
...
6106-GET-ABOUT-ME.
    ...
    PERFORM UNTIL WS-VALID-FIELD
        ...
        PERFORM 8000-DISPLAY-ROUTINE
        READ INPUT-FILE INTO UP-ABOUT-ME
            AT END SET WS-USER-WANT-TO-EXIT TO TRUE EXIT PERFORM
        END-READ
        IF FUNCTION LENGTH(FUNCTION TRIM(UP-ABOUT-ME)) > 200
...
*> Reads into WS-GENERIC-INPUT; enforces non-blank or allows DONE; uses current DISPLAY-MSG as prompt
6206-READ-REQUIRED-WITH-DONE.
    MOVE SPACES TO WS-GENERIC-INPUT
    PERFORM UNTIL FUNCTION LENGTH(FUNCTION TRIM(WS-GENERIC-INPUT)) > 0
        PERFORM 8000-DISPLAY-ROUTINE
...
6110-GET-OPTIONAL-EXP.
    MOVE 0 TO UP-NUM-EXP
    PERFORM VARYING J FROM 1 BY 1 UNTIL J > 3 OR WS-USER-WANT-TO-EXIT
        MOVE "Add Experience (enter DONE at any prompt to finish):" TO DISPLAY-MSG
        PERFORM 8000-DISPLAY-ROUTINE

        *> ----- TITLE (REQUIRED) -----
        MOVE J TO WS-J-TEXT
        STRING "Experience " FUNCTION TRIM(WS-J-TEXT) " - Title:"
            DELIMITED BY SIZE INTO DISPLAY-MSG
        PERFORM 6206-READ-REQUIRED-WITH-DONE
        IF WS-USER-WANT-TO-EXIT OR FUNCTION TRIM(WS-GENERIC-INPUT) = "DONE"
            EXIT PARAGRAPH
        END-IF
        MOVE WS-GENERIC-INPUT TO UP-EXP-TITLE(J)

        *> ----- COMPANY (REQUIRED) -----
        MOVE J TO WS-J-TEXT
        STRING "Experience " FUNCTION TRIM(WS-J-TEXT) " - Company/Organization:"
            DELIMITED BY SIZE INTO DISPLAY-MSG
        PERFORM 6206-READ-REQUIRED-WITH-DONE
        IF WS-USER-WANT-TO-EXIT OR FUNCTION TRIM(WS-GENERIC-INPUT) = "DONE"
            EXIT PARAGRAPH
        END-IF
        MOVE WS-GENERIC-INPUT TO UP-EXP-COMPANY(J)

        *> ----- DATES (REQUIRED) -----
        MOVE J TO WS-J-TEXT
        STRING "Experience " FUNCTION TRIM(WS-J-TEXT) " - Dates (e.g., Summer 2024):"
            DELIMITED BY SIZE INTO DISPLAY-MSG
        PERFORM 6206-READ-REQUIRED-WITH-DONE
        IF WS-USER-WANT-TO-EXIT OR FUNCTION TRIM(WS-GENERIC-INPUT) = "DONE"
            EXIT PARAGRAPH
        END-IF
        MOVE WS-GENERIC-INPUT TO UP-EXP-DATE(J)

        *> ----- DESCRIPTION (REQUIRED) -----
        MOVE J TO WS-J-TEXT
        STRING "Experience " FUNCTION TRIM(WS-J-TEXT) " - Description:"
            DELIMITED BY SIZE INTO DISPLAY-MSG
        PERFORM 6206-READ-REQUIRED-WITH-DONE
        IF WS-USER-WANT-TO-EXIT OR FUNCTION TRIM(WS-GENERIC-INPUT) = "DONE"
            EXIT PARAGRAPH
        END-IF
        MOVE WS-GENERIC-INPUT TO UP-EXP-DESC(J)

        ADD 1 TO UP-NUM-EXP
    END-PERFORM.

6111-GET-OPTIONAL-EDU.
    MOVE 0 TO UP-NUM-EDU
    PERFORM VARYING J FROM 1 BY 1 UNTIL J > 3
        ...
        MOVE WS-GENERIC-INPUT TO UP-EDU-DEGREE(J)

        MOVE J TO WS-J-TEXT
        STRING 'Education ' FUNCTION TRIM(WS-J-TEXT) ' - University/College:'
            DELIMITED BY SIZE INTO DISPLAY-MSG
        PERFORM 8000-DISPLAY-ROUTINE
        READ INPUT-FILE INTO WS-GENERIC-INPUT
            AT END SET WS-USER-WANT-TO-EXIT TO TRUE EXIT PERFORM
        END-READ
        MOVE WS-GENERIC-INPUT TO UP-EDU-UNI(J)

        MOVE J TO WS-J-TEXT
        STRING 'Education ' FUNCTION TRIM(WS-J-TEXT) ' - Years Attended (e.g., 2023-2025):'
            DELIMITED BY SIZE INTO DISPLAY-MSG
        PERFORM 8000-DISPLAY-ROUTINE
        READ INPUT-FILE INTO WS-GENERIC-INPUT
            AT END SET WS-USER-WANT-TO-EXIT TO TRUE EXIT PERFORM
        END-READ
        MOVE WS-GENERIC-INPUT TO UP-EDU-YEARS(J)
    END-PERFORM
...
6200-VALIDATE-PROFILE-FIELDS.
    MOVE UP-FIRST-NAME TO WS-TEMP-FIELD
    ...
    MOVE UP-LAST-NAME TO WS-TEMP-FIELD
    PERFORM 6210-ALPHA-ONLY
    IF DISPLAY-MSG NOT = SPACES
        MOVE WS-NAME-INVALID-MSG TO DISPLAY-MSG
        EXIT PARAGRAPH
        ...
        IF NOT WS-USER-WANT-TO-EXIT
            IF FUNCTION TRIM(WS-TEMP-FIELD) IS NUMERIC
                ...
                IF WS-VALIDATED-YEAR >= 1900 AND WS-VALIDATED-YEAR <= 2100
                    MOVE WS-VALIDATED-YEAR TO UP-GRAD-YEAR
                    SET WS-VALID-FIELD TO TRUE
                ELSE
                    MOVE UP-GRAD-YEAR-INVALID TO DISPLAY-MSG
                    PERFORM 8000-DISPLAY-ROUTINE
                END-IF
            ELSE
